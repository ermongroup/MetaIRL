...
def _worker_collect_one_path(G, max_path_length, scope=None, reset_arg=None, policy_context=None):
    G = _get_scoped_G(G, scope)
    path = rollout(G.env, G.policy, max_path_length, reset_args=reset_arg, policy_contexts=policy_context)
...
def sample_paths(
    ...
    if multi_task:
        args = [(max_path_length, scope, reset_arg[i], policy_context[i]) for i in range(len(reset_arg))]
        ...
        return singleton_pool.run_collect(
            _worker_collect_one_path,
            threshold=max_samples,
            args=(max_path_length, scope, reset_arg, policy_context),
            show_prog_bar=show_prog_bar,
            multi_task=multi_task,
        )
