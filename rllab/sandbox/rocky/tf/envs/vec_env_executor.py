...
class VecEnvExecutor(object):
    ...
    def step(self, action_n, reset_args=None, policy_contexts=None):
        if reset_args is None:
            policy_contexts = [None]*len(self.envs)
        ...
        for (i, done) in enumerate(dones):
            if done:
                obs[i] = self.envs[i].reset(reset_args=reset_args[i], policy_contexts=policy_contexts[i])
                self.ts[i] = 0
    ...
    def reset(self, reset_args=None, policy_contexts=None):
        if reset_args is not None:
            assert policy_contexts is not None
            results = [env.reset(reset_args=arg, policy_contexts=policy_context) for env, arg, policy_context in zip(self.envs, reset_args, policy_contexts)]
        else:
