...
class VectorizedSampler(BaseSampler):
    ...
    def obtain_samples(self, itr, reset_args=None, policy_contexts=None, return_dict=False):
        ...
        if reset_args is not None and type(policy_contexts) != list and type(policy_contexts)!=np.ndarray:
            policy_contexts = [policy_contexts]*self.n_envs

        n_samples = 0
        obses = self.vec_env.reset(reset_args, policy_contexts)
        ...
        while n_samples < self.algo.batch_size:
            ...
            t = time.time()
            next_obses, rewards, dones, env_infos = self.vec_env.step(actions, reset_args, policy_contexts)
