...
class InfoAIRL(SingleTimestepIRL):
    ...
    def __init__(self, env,
        ...
        with tf.variable_scope(name) as _vs:
            ...
            self.step = optimizer.apply_gradients(grads_and_vars_cent+grads_and_vars_context+grads_and_vars_reward)
            self.train_context_encoder_step = optimizer.apply_gradients(grads_and_vars_context)
    ...
    def fit(self, paths, expert_traj_batch=None, policy=None, batch_size=32, logger=None, lr=1e-3, train_context_only=False, **kwargs):
        meta_batch_size = self.meta_batch_size
        if self.fusion is not None:
            old_paths = self.fusion.sample_paths(n=len(paths[0]))
            self.fusion.add_paths(paths, expert_traj_batch)
        ...
        for it in TrainingIterator(self.max_itrs, heartbeat=5):
            ...
            if train_context_only:
                _ = tf.get_default_session().run(self.train_context_encoder_step, feed_dict=feed_dict)
            else:
