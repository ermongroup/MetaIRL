...
class RamFusionDistrCustom(FusionDistrManager):
    def __init__(self, buf_size, subsample_ratio=0.5):
        self.buf_size = buf_size
        self.buffer = {}
        self.subsample_ratio = subsample_ratio

    def add_paths(self, paths, expert_traj_batch, subsample=True):
        for key in paths.keys():
            expert_traj_key = str(expert_traj_batch[key])
            if expert_traj_key not in self.buffer:
                self.buffer[expert_traj_key] = []
            if subsample:
                subsample_paths = paths[key][:int(len(paths[key])*self.subsample_ratio)]
            else:
            ...
            while overflow > 0:
                ...
                probs = probs/float(np.sum(probs))
                pidx = np.random.choice(np.arange(N), p=probs)
                self.buffer[expert_traj_key].pop(pidx)
                overflow -= 1

    def sample_paths(self, expert_traj_batch, n):
        d = {}
        no_hit = False
        for ret_key, expert_traj in enumerate(expert_traj_batch):
            ...
            if expert_traj_key not in self.buffer or len(self.buffer[expert_traj_key]) == 0:
                ...
                pidxs = np.random.randint(0, len(self.buffer[expert_traj_key]), size=(n))
                d[ret_key] = [self.buffer[expert_traj_key][pidx] for pidx in pidxs]
        return d
