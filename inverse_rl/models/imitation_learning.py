...
class ImitationLearning(object, metaclass=Hyperparametrized):
    ...
    @staticmethod
    def _compute_path_probs_dict(paths, pol_dist_type=None, insert=True,
                            insert_key='a_logprobs'):
        """
        Returns a M x N x T matrix of action probabilities
        """
        path_probs = []
        for key in paths.keys():
            path_probs.append(ImitationLearning._compute_path_probs(paths[key], pol_dist_type=pol_dist_type,
                                                    insert=insert, insert_key=insert_key))
        return np.array(path_probs)
    ...
    def _insert_next_state(paths, pad_val=0.0):
        if type(paths) is dict:
            for key in paths.keys():
                paths[key] = ImitationLearning._insert_next_state(paths[key], pad_val=pad_val)
            return paths
        for path in paths:
    ...
    @staticmethod
    def extract_paths(paths, keys=('observations', 'actions'), stack=True, T=1):
        if type(paths) is dict:
            if stack:
                # return [np.stack([np.stack([t[key].reshape(T, -1) for t in paths[i]]).astype(np.float32) for i in paths.keys()]) for key in keys]
                try:
                    return [np.stack([np.stack([t[key].reshape(T, -1) for t in paths[i]]).astype(np.float32) for i in paths.keys()]) for key in keys]
                except:
                        for key in keys:
                            ...
                            for idx in paths.keys():
                                for t in paths[idx]:
                                    all_shape.add(str(t[key].reshape(T, -1).shape))
                            print(len(all_shape))
                        import pdb; pdb.set_trace()
            else:
                return [np.concatenate([np.concatenate([t[key].reshape(T, -1) for t in paths[i]]).astype(np.float32) for i in paths.keys()]) for key in keys]
        else:
            if stack:
                return [np.stack([t[key].reshape(T, -1) for t in paths]).astype(np.float32) for key in keys]
            else:
                return [np.concatenate([t[key].reshape(T, -1) for t in paths]).astype(np.float32) for key in keys]

    @staticmethod
    def sample_batch(*args, batch_size=32, warm_up=False, warm_up_idx=0):
...
class TrajectoryIRL(ImitationLearning):
    ...
    def eval_expert_probs(self, expert_paths, policy, insert=False, context=None):
        """
        Evaluate expert policy probability under current policy
        """
        if policy.recurrent:
            policy.reset([True]*len(expert_paths))
            expert_obs = ImitationLearning.extract_paths(expert_paths, keys=('observations',))[0]
        ...
        return ImitationLearning._compute_path_probs(expert_paths, insert=insert)
...
class SingleTimestepIRL(ImitationLearning):
    ...
    @staticmethod
    def extract_paths(paths, keys=('observations', 'actions'), stack=True, T=1):
        return ImitationLearning.extract_paths(paths, keys=keys, stack=stack, T=T)
    ...
    def eval_expert_probs(self, expert_paths, policy, insert=False, context=None):
        ...
        if isinstance(policy, np.ndarray):
            ...
            policy.reset([True]*len(expert_paths))
            expert_obs = ImitationLearning.extract_paths(expert_paths, keys=('observations',), stack=True)[0]
            if context is not None:
                expert_obs = np.concatenate((expert_obs, np.tile(np.expand_dims(context[i], axis=1), [1, expert_obs.shape[0], 1])), axis=-1)
            agent_infos = []
            for t in range(expert_obs.shape[1]):
                a, infos = policy.get_actions(expert_obs[:, t])
                agent_infos.append(infos)
            ...
            for i, path in enumerate(expert_paths):
                expert_obs = path['observations']
                if context is not None:
                    expert_obs = np.concatenate((expert_obs, np.tile(np.expand_dims(context[i], axis=0), [expert_obs.shape[0], 1])), axis=-1)
                actions, agent_infos = policy.get_actions(expert_obs)
                path['agent_infos'] = agent_infos
        return ImitationLearning._compute_path_probs(expert_paths, insert=insert)
