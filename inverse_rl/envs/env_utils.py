...
class CustomGymMonitorEnv(Monitor):
    @overrides
...
class RllabGymEnv(Env, Serializable):
    def __init__(self, env_name, wrappers=(), wrapper_args=(),
        ...
        if log_dir is None or record_log is False:
            ...
            self.env = CustomGymMonitorEnv(self.env, log_dir, video_callable=video_schedule, force=True)
    ...
    def reset(self):
        if self._force_reset and self.monitoring:
            ...
            assert isinstance(self.env, CustomGymMonitorEnv)
...
class CustomGymEnv(RllabGymEnv):
    ...
    def reset(self, reset_args=None, policy_contexts=None):
        if self._force_reset and self.monitoring:
            ...
            if recorder is not None:
                recorder.done = True
        kwargs = {'reset_args':reset_args, 'policy_contexts':policy_contexts}
